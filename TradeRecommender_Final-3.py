...
if not gm_mode:
    ...
    df["Current_Positional_WAR"] = df["position"].map(replacements)
    df["WAR Gained"] = df["Predicted WAR"] - df["Current_Positional_WAR"]
    gm_filtered = df[df["War Gained"] > 0]
    if gm_position != "All":Predicted WAR
        gm_filtered = gm_filtered[gm_filtered["position"] == gm_position]
    gm_filtered = gm_filtered.sort_values(by="benefit", ascending=False)
    st.dataframe(gm_filtered[["player_id", "team", "position", "Predicted WAR", "Current_Positional_WAR", "War Gained"]])
...
if gm_mode:
    ...
    filtered_df["benefit_score"] = (filtered_df["Predicted WAR"] * (filtered_df["years_control"]) * (1 - filtered_df["contract_value"] / max(filtered_df["contract_value"])) 
    gm_display = filtered_df[["player_id", "team", "position", "Predicted WAR", "contract_value", "years_control", "benefit_score"]]
