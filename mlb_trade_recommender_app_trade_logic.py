...
# Add filters
teams = ['ARI', 'LAD', 'SF', 'SD', 'COL', 'CHC', 'MIL', 'STL', 'PIT', 'CIN', 'ATL', 'PHI', 'MIA', 'NYM', 'WAS', 'NYY', 'BOS', 'BAL', 'TB', 'TOR', 'KC', 'DET', 'CWS', 'MIN', 'CLE', 'LAA', 'OAK', 'TEX', 'HOU', 'SEA']
df["Team"] = df["Team"]
...
# Select team initiating trade
trade_team = st.selectbox("Select Trade Team", teams)
...
# Filter to players that would be WAR upgrades over current starter on trade team
...
if mode == "Regular Mode":
    ...
    st.dataframe(improvement_df.sort_values(by="WAR_Upgrade", ascending=False)[cols + ['WAR_Upgrade']].head(10))
else:
    ...
    st.dataframe(improvement_df.sort_values(by="BenefitScore", ascending=False)[cols + ['WAR_Upgrade']].head(10))
