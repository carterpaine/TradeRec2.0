...
# Assign teams after creating DataFrame
import random

mlb_teams = [
    'ARI', 'LAD', 'SF', 'SD', 'COL', 'CHC', 'MIL', 'STL', 'PIT', 'CIN',
    'ATL', 'PHI', 'MIA', 'NYM', 'WAS', 'NYY', 'BOS', 'BAL', 'TB', 'TOR',
    'KC', 'DET', 'CWS', 'MIN', 'CLE', 'LAA', 'OAK', 'TEX', 'HOU', 'SEA'
]
df["Team"] = np.random.choice(mlb_teams, size=len(df))

# Compute team starter WAR benchmarks
team_position_war = {
    (team, pos): round(np.random.uniform(0.0, 4.0), 2)
    for team in mlb_teams for pos in df["Type"].unique()
}
df["TeamStarterWAR"] = df.apply(lambda row: team_position_war.get((row["Team"], row["Type"]), 0.0), axis=1)
df["WAR_Upgrade"] = df["WAR"] - df["TeamStarterWAR"]

# Add filters
positions = sorted(df["Type"].unique())
selected_team = st.selectbox("Select Team", ["All"] + mlb_teams)
selected_position = st.selectbox("Select Position/Type", ["All"] + positions)
trade_team = st.selectbox("Select Trade Team", mlb_teams)
...
# Filter to upgrade candidates
improvement_df = df[df["Team"] != trade_team]
improvement_df = improvement_df[improvement_df.apply(
    lambda row: row["WAR"] > team_position_war.get((trade_team, row["Type"]), 0), axis=1)]
...
if mode == "Regular Mode":
    ...
    st.dataframe(df.sort_values(by="WAR", ascending=False)[cols].head(10))
else:
    ...
    st.dataframe(df.sort_values(by="BenefitScore", ascending=False)[cols].head(10))
