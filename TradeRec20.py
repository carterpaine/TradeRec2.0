...
def generate_synthetic_dataset(n_players=900):
    ...
    data = {
        ...
        "Predicted WAR": np.round(np.random.normal(2.0, 1.5, n_players), 2),
...
if not gm_mode:
    ...
    top10 = filtered_df.sort_values(by="Predicted WAR", ascending=False).head(10)
    st.subheader("Top 10 Players by Predicted WAR")
    st.dataframe(top10[["player_id", "team", "position", "BA", "OBP", "SLG", "OPS", "SB", "HR", "RBI", "Predicted WAR"]])
else:
    st.subheader(f"GM Mode: Value Over Replacement on {gm_team}")
    replacements = df[df["team"] == gm_team].groupby("position")["Predicted WAR"].max().to_dict()
    df["Current Positional Value"] = df["position"].map(replacements)
    df["WAR Gained"] = df["Predicted WAR"] - df["Current Positional Value"]
    gm_filtered = df[df["WAR Gained"] > 0]
    if gm_position != "All":
        gm_filtered = gm_filtered[gm_filtered["position"] == gm_position]
    gm_filtered = gm_filtered.sort_values(by="WAR Gained", ascending=False)
    st.dataframe(gm_filtered[["player_id", "team", "position", "Predicted WAR", "Current Positional Value", "WAR Gained"]])
...
with col1:
    ...
    fig = px.scatter(filtered_df, x="age", y="Predicted WAR", color="position", hover_data=["player_id"])
    st.plotly_chart(fig, use_container_width=True)

with col2:
    st.markdown("**Team Contract Value vs WAR**")
    fig = px.scatter(filtered_df, x="contract_value", y="Predicted WAR", color="team", size="years_control")
    st.plotly_chart(fig, use_container_width=True)

if gm_mode:
    st.subheader("GM Mode Insights")
    filtered_df["Value_score"] = (1 - df["Predicted WAR"] * df["years_control"]) * (df["contract_value"] / df["contract_value"].max())
    gm_display = filtered_df[["player_id", "team", "position", "Predicted WAR", "contract_value", "years_control", "Value_score"]]
    st.dataframe(gm_display.sort_values("Value_score", ascending=False))
